...
function New-RancherServer {
  ...
  $existingServer = hcloud server list -o noheader | Select-String "eutlas-rancher"
    
  if (-not $existingServer) {
    # Check for SSH key
    $sshKey = hcloud ssh-key list -o noheader | Select-String "eutlas-admin"
    if (-not $sshKey) {
      Write-Warning "No SSH key found. Creating one..."
      if (-not (Test-Path "$env:USERPROFILE\.ssh\id_ed25519")) {
        ssh-keygen -t ed25519 -f "$env:USERPROFILE\.ssh\id_ed25519" -N '""' -C "eutlas-admin"
      }
      $pubkey = Get-Content "$env:USERPROFILE\.ssh\id_ed25519.pub"
      hcloud ssh-key create --name eutlas-admin --public-key "$pubkey"
    }
        
    # Note: cx33 is the newer naming for 4 vCPU, 8GB RAM servers
    hcloud server create `
      --name eutlas-rancher `
      --type cx33 `
      --image ubuntu-22.04 `
      --location $RancherLocation `
      --ssh-key eutlas-admin
        
    $rancherIP = hcloud server ip eutlas-rancher
    ...
    Write-Host "   ssh root@$rancherIP"
    Write-Host ""
    Write-Host "2. Run these commands:"
    Write-Host "   apt-get update && apt-get install -y docker.io"
    Write-Host "   systemctl enable docker && systemctl start docker"
    ...
    Write-Host "4. Access: https://rancher.$Domain"
